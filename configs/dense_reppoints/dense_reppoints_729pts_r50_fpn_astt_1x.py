...
model = dict(
    ...
    bbox_head=dict(
        ...
        loss_pts_angle_init=dict(type='ChamferLoss2D', use_cuda=True, loss_weight=1, eps=1e-8),
        loss_pts_refine=dict(type='ChamferLoss2D', use_cuda=True, loss_weight=1.0, eps=1e-8),
        loss_pts_angle_refine=dict(type='ChamferLoss2D', use_cuda=True, loss_weight=2.0, eps=1e-8),
        loss_mask_score_init=dict(type='CrossEntropyLoss', use_sigmoid=True, loss_weight=1.0),
        transform_method='minmax'))
# training and testing settings
train_cfg = dict(
    init=dict(
        assigner=dict(type='PointBoxAssigner', scale=4, pos_num=1),
        allowed_border=-1,
        pos_weight=-1,
        mask_size=56,
        debug=False,
        dist_sample_thr=2,
        sample_func='distance_sample_pts'),
    refine=dict(
...
log_level = 'INFO'
work_dir = './work_dirs/dense_reppoints_729pts_r50_fpn_astt_1x'
load_from = None
resume_from = None
workflow = [('train', 1)]
